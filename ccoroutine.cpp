...
void* CCorutine::operator new(size_t nSize) {
    return g_malloc(nSize);
}
void CCorutine::operator delete(void* p) {
    g_free(p);
}
...
bool CCorutine::IsCoroutineError(time_t tmNow) {
    if (CCoroutineUnLikely(m_state == CoroutineState_Suspend || m_state == CoroutineState_Running))
        //5s
